...
base_dir = "C:/Users/cksdn/Downloads/visnet_mine/data/street2shop/images/"
...
def get_image(id_list):
    ...
    for id_triplet in id_list:
        img = Image.open(base_dir + str(id_triplet[0]) + ".jpg")
        img = img.resize((299, 299))
        img = np.array(img, dtype=np.float32)
        image_q.append(img)
        img = Image.open(base_dir + str(id_triplet[1]) + ".jpg")
        img = img.resize((299, 299))
        img = np.array(img, dtype=np.float32)
        image_p.append(img)
        img = Image.open(base_dir + str(id_triplet[2]) + ".jpg")
        img = img.resize((299, 299))
        img = np.array(img, dtype=np.float32)
        image_n.append(img)
